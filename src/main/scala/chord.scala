...
class Node(idc: Int, mc: Int) extends Actor {
    ...
    var n1: NodeInfo = new NodeInfo(self, nodeId)
      ...
      if (n1.node != self) {
        ...
        n1Succ = successor
  		...
  		queryInterval = (n1.nodeId - 1) to n1Succ.nodeId
  	}
  ...
  def init_finger_table(n1: NodeInfo) = {
		...
		if (successor.node != self) {
			...
			val success = Await.result(future, timeout.duration).asInstanceOf[NodeInfo]
		}
		else
  ...
  def update_others() = {
	...
	def update_finger_table(s: NodeInfo, i: Int): Unit = {
		var queryInterval = nodeId to (finger(i).node.nodeId - 1)
		if (queryInterval.contains(s.nodeId)) {
			finger(i).node = s
			var p = predecessor
			if (p.node != self)
    ...
    if (n1.node != self) {
 	...
 	case ClosestPrecedingFinger(id) =>
 		val queryInterval = (nodeId - 1) to (id - 1)
  		var node = -1
  		for (i <- m to 1 by -1) {		
				if (queryInterval.contains(finger(i).node.nodeId)) {
